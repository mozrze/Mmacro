...
RunUpdateScript_Inner(zipURL) {
    ...
    script .= "$form.ClientSize = New-Object System.Drawing.Size(420, 130)`r`n"
    script .= "$form.StartPosition = 'CenterScreen'`r`n"
    script .= "$form.FormBorderStyle = 'FixedDialog'`r`n"
    script .= "$form.MaximizeBox = $false`r`n"
    script .= "$form.MinimizeBox = $false`r`n"
    script .= "$form.ControlBox = $false`r`n"
    ...
    script .= "$label.Location = New-Object System.Drawing.Point(20, 20)`r`n"
    script .= "$form.Controls.Add($label)`r`n"
    script .= "$progress = New-Object System.Windows.Forms.ProgressBar`r`n"
    script .= "$progress.Location = New-Object System.Drawing.Point(20, 55)`r`n"
    script .= "$progress.Size = New-Object System.Drawing.Size(380, 22)`r`n"
    script .= "$progress.Style = 'Marquee'`r`n"
    script .= "$progress.MarqueeAnimationSpeed = 25`r`n"
    script .= "$form.Controls.Add($progress)`r`n"
    script .= "$form.Show()`r`n"
    script .= "[System.Windows.Forms.Application]::DoEvents()`r`n"
    script .= "`r`n"
    script .= "try {`r`n"
    script .= "    Write-Host 'Downloading...'`r`n"
    script .= "    $state = @{ Done = $false; Error = $null }`r`n"
    script .= "    $client = New-Object System.Net.WebClient`r`n"
    script .= "    $client.Headers.Add('User-Agent', 'TD-Macro-Updater')`r`n"
    script .= "    $client.add_DownloadProgressChanged({ param($sender, $event); $progress.Style = 'Continuous'; $progress.Value = $event.ProgressPercentage; $label.Text = ('Downloading update... {0}%' -f $event.ProgressPercentage) })`r`n"
    script .= "    $client.add_DownloadFileCompleted({ param($sender, $event); $state.Error = $event.Error; $state.Done = $true })`r`n"
    script .= "    $client.DownloadFileAsync([System.Uri]::new('" . safeURL . "'), '" . safeZip . "')`r`n"
    script .= "    while (-not $state.Done) { [System.Windows.Forms.Application]::DoEvents(); Start-Sleep -Milliseconds 50 }`r`n"
    script .= "    $client.Dispose()`r`n"
    script .= "    if ($state.Error) { throw $state.Error }`r`n"
    ...
    script .= "    $label.Text = 'Update failed: ' + $_.Exception.Message`r`n"
    script .= "    $label.ForeColor = [System.Drawing.Color]::FromArgb(220, 80, 80)`r`n"
    script .= "    $progress.Style = 'Continuous'`r`n"
    script .= "    $progress.Value = 0`r`n"
    script .= "    [System.Windows.Forms.Application]::DoEvents()`r`n"
